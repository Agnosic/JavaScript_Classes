...
http.createServer (function (request, response) {
    ...
    if (url_parts.pathname == '/date' && request.method == 'GET') {  // Processing the form content, if the relative URL is '/ submit'
      response.writeHead(200, {"Content-Type": "text/plain; charset=utf-8"});
      let shanghaiTime = new Date().toLocaleString("en-US", {timeZone: "Asia/Shanghai"});
      var date = new Date(shanghaiTime);
      response.write(date.toUTCString());
      response.end(); // Sending the answer
      console.log("The server sent the '"+date+"' text to the browser"); 
    }
    ...
    else if (url_parts.pathname == '/submit' && request.method == 'GET') {  // Processing the form content, if the relative URL is '/ submit'
      var welcome = "\nWitaj " + url_parts.query['imie'];
